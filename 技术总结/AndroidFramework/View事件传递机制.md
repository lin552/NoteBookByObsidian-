...
- 若返回 `true`：​**拦截事件**，后续同一事件链的所有事件（如 `ACTION_MOVE`、`ACTION_UP`）不再传递给子 `View`，由当前 `ViewGroup` 自身处理（调用 `onTouchEvent`）。

##### 4）如何判断是哪个子view拦截了事件
`ViewGroup` 遍历所有子 `View`，找到**最符合条件的子 `View`**​（即触摸点在其范围内，且未被禁用），调用其 `dispatchTouchEvent`。
- 子 `View` 若 `onTouchEvent` 返回 `true`（消费事件），则成为当前事件链的“责任方”，后续事件由它处理。
- 子 `View` 若 `onTouchEvent` 返回 `false`（未消费），`ViewGroup` 会尝试传递给下一个子 `View`，若所有子 `View` 都未消费，则由 `ViewGroup` 自身处理。
