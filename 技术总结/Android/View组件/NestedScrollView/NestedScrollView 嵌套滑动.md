...
#### 三、工作原理
##### 1）嵌套滑动机制
- 实现 `NestedScrollingParent2` 和 `NestedScrollingChild2` 接口，支持与 `RecyclerView`、`ListView` 等子视图协同处理滚动事件，避免滑动冲突
- 通过 `startNestedScroll` 和 `dispatchNestedPreScroll` 等方法实现父子视图间的滚动事件传递
##### 2）事件分发流程
###### ① `Pre-Scroll`
子控件滑动前将增量传递给父控件
###### ② 消费阶段
